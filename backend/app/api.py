...
app.add_middleware(
    ...
    allow_headers=["Content-Type", "X-Scrappify-Key"],
)
...
def dashboard():
    with connection() as conn:
        ...
        active_job = conn.execute(
            """SELECT * FROM scrape_jobs WHERE status IN ('queued','running')
...
def list_jobs(limit: int = Query(default=25, ge=1, le=100)):
    with connection() as conn:
        return conn.execute(
            "SELECT * FROM scrape_jobs ORDER BY created_at DESC LIMIT %s",
            (limit,),
        ).fetchall()
...
def patch_product(product_id: UUID, payload: ProductPatch):
...
def ai_enrich(payload: IdList):
    return enrich_many(payload.product_ids)
...
def shopify_sync(payload: IdList):
    ...
    for product_id in payload.product_ids:
        try:
            synced.append(sync_product(product_id))
...
def export_shopify_csv(ids: str = ""):
    selected = [UUID(value) for value in ids.split(",") if value] if ids else []
    with connection() as conn:
        if selected:
            products = conn.execute(
                "SELECT * FROM products WHERE id = ANY(%s) ORDER BY updated_at DESC",
                (selected,),
            ).fetchall()
        else:
            products = conn.execute("SELECT * FROM products ORDER BY updated_at DESC").fetchall()
